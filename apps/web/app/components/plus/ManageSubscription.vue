...
const viaDiscord = computed(() => props.plan?.source === "discord");
...
const nextUp = computed(() => { if (!subscribed.value) return null; if (viaDiscord.value) return "Billed through Discord. Change or cancel it in Discord under User Settings, Subscriptions."; if (canceling.value) return `Ends on ${renewLabel.value}. You keep everything until then.`; if (pending.value) return `Switches to ${tierName(pending.value)} on ${renewLabel.value}.`; if (renewLabel.value) return `Renews on ${renewLabel.value}.`; return null; });
...
<div class="card manage"><div class="manage__head"><div><h2 class="card-title">Subscription</h2><p class="meta">{{ subscribed ? "Ranked World " + tierName(current) : "Free plan" }}</p></div><span v-if="subscribed" class="tag" :data-plan="current">{{ tierName(current) }}</span></div><p v-if="nextUp" class="meta manage__next">{{ nextUp }}</p><p v-if="plan?.status === 'past_due'" class="manage__warn">
    ...
    </p><p v-if="error" class="manage__warn">{{ error }}</p><div v-if="!subscribed" class="manage__actions"><NuxtLink class="btn btn--brand" to="/subscribe">See the plans</NuxtLink></div><div v-else-if="!viaDiscord" class="manage__actions"><NuxtLink v-if="!canceling" class="btn btn--brand" to="/subscribe">
